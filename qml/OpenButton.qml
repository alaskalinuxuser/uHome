...
LomiriShape {
    ...
    Icon {
        id: icon
        visible: {if (iconSource!=""){false}else{true}}
        anchors{
            left: parent.left
    ...
    Image{
    id: iconImg
    visible: {if (iconSource==""){false}else{if (imgColored){false}else{true}}
        }
            anchors{
                left: parent.left
    ...
    ColorOverlay{
    id:  iconImgcolor
    visible: {if (iconSource==""){false}else{if (imgColored){true}else{false}}
        }
            anchors{
                left: parent.left
                leftMargin: units.gu(0.5)
